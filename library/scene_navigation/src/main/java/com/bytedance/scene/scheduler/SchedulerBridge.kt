...
class SchedulerBridge(
    ...
    private var isSett = false
    fun runNext(operation: ForcibleTerminationOperation): SchedulerNode {
        if (this.isSett) {
            throw IllegalArgumentException("Task is already invoke runNext")
        }
        ...
        return SchedulerNode(
            ...
            sharedEnvironment.terminationSignal,
            operation
        ).also { scheduler ->
            sharedEnvironment.setStartSignal {
                scheduler.run()
            }
        }
    }
