package com.bytedance.scene.navigation.post.mix;
...
public class Mix_Push_Post_Pop_Post_Lifecycle_Order {
    ...
    public void test() {
        ...
        assertEquals(State.STARTED, navigationScene.getSceneList().get(0).getState());

        navigationScene.pop(new PopOptions.Builder().setUsePost(true).build());

        //it will force previous push post be executed immediately, then pause top scene
        ...
        //destroy top scene, return to previous scene
        shadowOf(getMainLooper()).runOneTask();//execute Handler posted task
        assertEquals(1, navigationScene.getSceneList().size());
        assertEquals(State.RESUMED, navigationScene.getSceneList().get(0).getState());
    }
